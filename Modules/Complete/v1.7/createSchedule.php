...
<head>
    ...
    function do_schedule()
    {
        ...
        fclose($excel_file);
        @mkdir(SCHEDULES_DIR.$_SESSION['DIRECTORY']);
        $excel_file = file_get_contents($_SESSION['DIRECTORY'].EXCEL_FILE);
        file_put_contents(SCHEDULES_DIR.$_SESSION['DIRECTORY'].EXCEL_FILE, $excel_file);


        //Create graphical schedule
        $schedule = serialize($schedule);
        file_put_contents($_SESSION['DIRECTORY'].SCHEDULE_FILE, $schedule);
        $schedule_file = file_get_contents($_SESSION['DIRECTORY'].SCHEDULE_FILE);
        file_put_contents(SCHEDULES_DIR.$_SESSION['DIRECTORY'].SCHEDULE_FILE, $schedule_file);
        
        build_schedule_table();
        
        //Get file for building schedule
        $build_schedule = file_get_contents("build_schedule.php");
        file_put_contents(SCHEDULES_DIR.$_SESSION['DIRECTORY']."build_schedule.php", $build_schedule);
