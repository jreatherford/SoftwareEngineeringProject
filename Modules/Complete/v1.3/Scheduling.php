...
include_once 'functions.php'; //database functions
...
class Schedule_Generator
{
    ...
    public function __construct()
    {
        ...
        $this->file_info = @file_get_contents($_SESSION['DIRECTORY'] . INFO_FILE);
        ...
        if($this->file_info == FALSE)
        {
            ...
            throw new Exception($err1 . $_SESSION['DIRECTORY'] . INFO_FILE . $err2);
        }
        ...
        if(get_class($this->file_info) != "File_Info")
        {
            ...
            throw new Exception($_SESSION['DIRECTORY'] . INFO_FILE . $error);
        }
               ...
               if( $item->course->section != "internet" )
                {
                    ...
                    $this->mark_reserved( $item->room, $scheduled );
    ...
    private function get_valid_times( &$item )
    {
        ...
        if( $item->course->section == "day" )
            @$valid_times = array_merge( $times['morning'], $times['midday'] );
        else //course section is "night"
            @$valid_times = $times['evening'];
...
{
    $fac = file_get_contents($_SESSION['DIRECTORY'].FACULTY_FILE);
    return unserialize($fac);
}

function Get_Faculty_List_DB()
{
    ////get faculty from database
    //-> shadow database list with list of new _Faculty objects

    $fac = array();
    $faculty_members = Get_users();
    foreach( $faculty_members as $f )
    {
        $_f = new _Faculty;
        $_f->name = $f['name'];
        $_f->min_hours = $f['hours'];
        $_f->seniority = $f['year_of_service'];
        $_f->current_hours = 0;
        $fac[] = $_f;
    }
    ...
    $admin = Get_adminid('1');
    $_f = new _Faculty;
    $_f->name = $admin['admin_name'];
    $_f->min_hours = $admin['hour'];
    $_f->seniority = $admin['year_of_service'];
    $_f->current_hours = 0;
    $fac[] = $_f;

    //"Cache" to stop multiple accesses to the database from slowing algorithm
    $fac = serialize($fac);
    file_put_contents($_SESSION['DIRECTORY'].FACULTY_FILE, $fac);
}

function Get_Prefs()
{
    $prefs = unserialize( @file_get_contents( $_SESSION['DIRECTORY'].PREF_FILE ) );
...
function compute_weights($error_list)
{
    ...
    foreach( $error_list as $error )
    {
        if( preg_match("/preferred time/",$error) )
            $score += $_SESSION['ERROR_WEIGHTS']['UNWANTED_TIME'];
        elseif( preg_match("/preference/", $error) )
            $score += $_SESSION['ERROR_WEIGHTS']['UNSATISFIED_PREF'];
        else
        {
            $level = get_course_level($error);
            if( $level < "200" )
                $score += $_SESSION['ERROR_WEIGHTS']['100_LEVEL'];
            elseif( $level < "300" )
                $score += $_SESSION['ERROR_WEIGHTS']['200_LEVEL'];
            elseif( $level < "400" )
                $score += $_SESSION['ERROR_WEIGHTS']['300_LEVEL'];
            else //400+
                $score += $_SESSION['ERROR_WEIGHTS']['400_LEVEL'];
        }
    }
...
function Generate_Schedule()
{
    Get_Faculty_List_DB();
    $best = INFINITY;
    for( $i=0; $i<$_SESSION['NUM_SCHEDULE_ATTEMPTS']; $i++ )
    {
