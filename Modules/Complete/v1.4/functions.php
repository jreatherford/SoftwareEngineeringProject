...
function Get_singleuser($username)
{
    $conn = Get_connection();
...
//******************************************************************************
//Puclic Functions: Insert_file
//This function is to intsert information from file into database
//******************************************************************************
function Insert_file($username, $name, $password, $yos, $email, $hour)
{
    ...
    $stmt = $conn->prepare('insert into users ' .
            '(username, name, password, hours, email, year_of_service)
                values (?,?,?,?,?,?)');
    $stmt->execute(array($username, $name, $password, $hour, $email, $yos));
}
...
function Update_row($row)
{
    ...
    $stmt = $conn->prepare('update users set username=?, password=?,
        email=?, hours=?, year_of_service=? where user_id=?');
    $stmt->execute(array($row['username'], $row['password'], $row['email'],
        $row['hours'], $row['year_of_service'], $row['user_id']));
}
...
function Update_admin($rowid)
{
    ...
    $stmt = $conn->prepare('update admin set admin_name=?, password=?,
        admin_email=?, hour=?, year_of_service=? where admin_id=?');
    $stmt->execute(array($rowid['admin_name'], $rowid['password'],
        $rowid['admin_email'], $rowid['hour'], $rowid['year_of_service'],
        $rowid['admin_id']));
}
    ...
    if($password != '')
    {
        ...
        else if(preg_match("/[A-Za-z]/",$password[0]))
        {
            if(preg_match("/\d/",substr($password,1)) == "1"
                    && preg_match("/!|\?|,/",substr($password,1)) == "1"
                    && preg_match("/[^0-9A-Za-z!\?,]/",substr($password,1)) == "0")
            {
                echo "true";
                return true;
            }
...
//******************************************************************************
//Puclic Functions: Login
//This function is to check the password and username right or not.
//******************************************************************************
...
//******************************************************************************
//Puclic Functions: Get_ename
//This function is to create a new username accordig to their email address, the
//part of in the front of @ symbol.
//******************************************************************************
function Get_ename($email)
{
