...
function build_pref_table($location, $instructor)
{
    ...
    //get the courses
    $course_list = unserialize(@file_get_contents($_SESSION['DIRECTORY'].INFO_FILE));
    ...
    $pref_list = (@file_get_contents($_SESSION['DIRECTORY'].PREF_FILE));
    if ($pref_list === FALSE)
        $pref_list = array();
    else
...
function print_table($location, $pref_list, $course_list, $instructor, $message)
{
    ...
    {
        //if this is someone else's pref, skip it
        if ($curr_pref->faculty_name != $instructor->name)
            continue;
    ...
    //update the file
    @file_put_contents($_SESSION['DIRECTORY'].PREF_FILE, serialize($pref_list));
...
function check_added_courses (&$prefs, $instructor, &$message, $curr_hours)
{
    ...
    if (substr($new_course,(strlen($new_course)-2)) == ".i")
     ...
     if ($in_list == false)
     {
        ...
        //update the file
        file_put_contents($_SESSION['DIRECTORY'].PREF_FILE, serialize($prefs));
