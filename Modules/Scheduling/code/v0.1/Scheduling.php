...
function compute_weights($error_list)
{
    ...
    $_ENV['ERROR_WEIGHTS']['100_LEVEL'];
    $_ENV['ERROR_WEIGHTS']['200_LEVEL'];
    $_ENV['ERROR_WEIGHTS']['300_LEVEL'];
    $_ENV['ERROR_WEIGHTS']['400_LEVEL'];
    $_ENV['ERROR_WEIGHTS']['FACULTY_PREF'];

    $score = 0;
    foreach( $error_list as $error )
    {
        if( preg_match("/preference/", $error) )
            $score += $_ENV['ERROR_WEIGHTS']['FACULTY_PREF'];
        else
        {
